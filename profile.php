...
$sendemail = false;
$result = "";
$email_settings = array("reward_text" => "", "newsletter" => "");

if(isset($_GET['tab'])){
	$getTab = (int) $_GET['tab'];
	if($getTab < 1 || $getTab > 5 ){
		header("Location: ".PATH."/profile?tab=1"); exit;
	}else{
		$tab = (string) $getTab;
	}
}elseif(isset($_POST['tab'])){
	$postTab = (int) $_POST['tab'];
	if($postTab < 1 || $postTab > 5 ){
		header("Location: ".PATH."/profile?tab=1"); exit;
	}else{
		$tab = (string) $postTab;
	}
...
case "2":
	...
	if(isset($_POST['motto'])){
		...
		if(isset($_POST['visibility']) && $_POST['visibility'] == "EVERYONE"){ $visibility = 1; }else{ $visibility = 0; }
		if(isset($_POST['emailMiniMailAlertEnabled']) && $_POST['emailMiniMailAlertEnabled'] == "true"){ $minimail_alert = 1; }else{ $minimail_alert = 0; }
		if(isset($_POST['emailFriendRequestAlertEnabled']) && $_POST['emailFriendRequestAlertEnabled'] == "true"){ $request_alert = 1; }else{ $request_alert = 0; }
		if(isset($_POST['showOnlineStatus']) && $_POST['showOnlineStatus'] == "true"){ $showstatus = 1; }else{ $showstatus = 0; }
...
case "3":
	...
	if(isset($_POST['email'])){
		$postedCaptcha = isset($_POST['captcha']) ? $_POST['captcha'] : "";
		if(((isset($_SESSION['register-captcha-bubble']) && $_SESSION['register-captcha-bubble'] == $postedCaptcha) && !empty($_SESSION['register-captcha-bubble'])) || $settings->find("site_capcha") == "0"){
			...
			if(isset($_POST['directemail']) && $_POST['directemail'] == "on"){ $newsletter = "checked=\"checked\""; }else{ $newsletter = ""; }
			if($pass1_hash == $user->user("password") && $formatted_dob == $user->user("birth")){
				...
				if($email_check == "1"){
					if(isset($_POST['directemail']) && $_POST['directemail'] == "on"){ $newsletter = "1"; }else{ $newsletter = "0"; }
	...
	$mailer->sendSimpleMessage($to,$subject,$html);
		...
		$serverdb->query("UPDATE ".PREFIX."users SET email_verified = '1' WHERE id = '".$user->id."' LIMIT 1");
	}
break;
case "4":
...
<p class="last">
<?php echo $lang->loc['email.verify.message'].$email_settings['reward_text']; ?>
</p>

<div class="settings-buttons">
<a href="#" class="new-button" style="display: none" id="confirmform-submit"><b><?php echo $lang->loc['activate.my.email']; ?></b><i></i></a>
<noscript><input type="submit" value="<?php echo $lang->loc['activate.my.email'].$email_settings['reward_text']; ?>" name="save" class="submit" /></noscript>
...
<?php if($user->user("email_verified") == 0 && $settings->find("email_verify_enabled") == 1){ ?>
<script type="text/javascript">
