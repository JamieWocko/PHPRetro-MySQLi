...
function installer_array_value($array, $key, $default = ''){
	return (is_array($array) && isset($array[$key])) ? $array[$key] : $default;
}

function installer_remote_addr(){
	return isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1';
}

function writeConfig($db){
$dbPrefix = installer_array_value($db, 'db_prefix');
$dbServer = installer_array_value($db, 'db_server', 'mysql');
$dbHost = installer_array_value($db, 'db_host', 'localhost');
$dbPort = installer_array_value($db, 'db_port');
$dbUsername = installer_array_value($db, 'db_username');
$dbPassword = installer_array_value($db, 'db_password');
$dbName = installer_array_value($db, 'db_name');
$data = 
'<?php
/*================================================================+\
|| # PHPRetro - An extendable virtual hotel site and management
...
'.'$'.'conn[\'main\'][\'prefix\'] = "'.$dbPrefix.'";
'.'$'.'conn[\'main\'][\'server\'] = "'.$dbServer.'"; //mysql, pgsql, sqlite, or mssql
'.'$'.'conn[\'main\'][\'host\'] = "'.$dbHost.'"; //filename for SQLite
'.'$'.'conn[\'main\'][\'port\'] = "'.$dbPort.'";
'.'$'.'conn[\'main\'][\'username\'] = "'.$dbUsername.'";
'.'$'.'conn[\'main\'][\'password\'] = "'.$dbPassword.'";
'.'$'.'conn[\'main\'][\'database\'] = "'.$dbName.'";
...
'.'$'.'conn[\'server\'][\'server\'] = "'.$dbServer.'"; //mysql, pgsql, sqlite, or mssql
'.'$'.'conn[\'server\'][\'host\'] = "'.$dbHost.'"; //filename for SQLite
'.'$'.'conn[\'server\'][\'port\'] = "'.$dbPort.'";
'.'$'.'conn[\'server\'][\'username\'] = "'.$dbUsername.'";
'.'$'.'conn[\'server\'][\'password\'] = "'.$dbPassword.'";
'.'$'.'conn[\'server\'][\'database\'] = "'.$dbName.'";
?>';
...
$query[] = "DROP TABLE IF EXISTS `".PREFIX."security_log`;";
...
$query = array();
$query[] = "insert into `".PREFIX."campaigns` values('1','%path%/housekeeping/','http://localhost/c_images/hot_campaign_images_gb/cb_country_160x60.gif','Please change','These are sample campaigns, please edit them via housekeeping.','1'),
 ('2','%path%/articles','http://localhost/c_images/hot_campaign_images_gb/easter_160x60.gif','News','See the latest news in our hotel.','1'),
 ('3','http://code.google.com/p/phpretro/issues/list','http://localhost/c_images/hot_campaign_images_gb/uk_newsletter_160x70.gif','Found a bug?','Submit it to our Google Codes page!','1');";
...
$query[] = "insert into `".PREFIX."settings` values('site_name','Retro Hotel','".$loc['settings.site_name']."','".$loc['settings.site_name']."','textbox','','site','0','".$loc['settings.category.main']."'),
 ...
 ('site_c_images_path','http://localhost/c_images','".$loc['settings.site_c_images_path']."','".$loc['settings.site_c_images_path.desc']."','textbox','','site','4','".$loc['settings.category.paths']."'),
 ...
 ('hotel_ip','".installer_remote_addr()."','".$loc['settings.hotel_ip']."','".$loc['settings.hotel_ip.desc']."','textbox','','hotel','1','".$loc['settings.category.paths']."'),
 ...
 ('site_hotel_image','htlview_au.png','".$loc['settings.site_hotel_image']."','".$loc['settings.site_hotel_image.desc']."','textbox','','content','9','".$loc['settings.category.home']."'),
...
function installDB(){
	@set_time_limit(0);
	global $lang;
	define('SHORTNAME', installer_array_value($_SESSION['settings'], 's_site_shortname', 'Retro'));
	define('PATH', installer_array_value($_SESSION['settings'], 's_site_path', 'http://localhost'));
	define('PREFIX', installer_array_value($_SESSION['settings'], 'db_prefix'));
	...
	$GLOBALS['serverdb'] = $db;
	$runQuery = function($query, $label = null) use ($db){
		$result = $db->query($query);
		if($result === false){
			echo '<div class="installer-error"><div class="rounded rounded-red">';
			if(!empty($label)){ echo htmlspecialchars($label, ENT_COMPAT, 'UTF-8').'<br />'; }
			echo htmlspecialchars($db->get_error(), ENT_COMPAT, 'UTF-8').'<br />';
			echo '<code>'.htmlspecialchars($query, ENT_COMPAT, 'UTF-8').'</code>';
			echo '</div></div>';
			return false;
		}
		return true;
	...
	foreach($queries as $query){
		if(preg_match('/^\s*CREATE\s+TABLE\s+`([^`]+)`/i', $query, $matches)){
			if(!$runQuery("DROP TABLE IF EXISTS `".$matches[1]."`;", $lang->loc['creating.tables'])){ return false; }
		}
		if(!$runQuery($query, $lang->loc['creating.tables'])){ return false; }
	}
	echo "<strong>".$lang->loc['inserting.queries']."...</strong><br />";
	$queries = installQueries();
	foreach($queries as $query){
		if(!$runQuery($query, $lang->loc['inserting.queries'])){ return false; }
	}
	...
	foreach($_SESSION['settings'] as $key => $value){
		if(substr($key, 0, 2) == "s_"){
			...
			if(!$runQuery("UPDATE ".PREFIX."settings SET value = '".$value."' WHERE id = '".$key."' LIMIT 1", $lang->loc['updating.settings'])){ return false; }
		}
	}
	...
	$data->alter1();
	if(!empty($db->get_error())){
		echo '<div class="installer-error"><div class="rounded rounded-red">'.htmlspecialchars($db->get_error(), ENT_COMPAT, 'UTF-8').'</div></div>';
		return false;
	}
	$data->delete1();
	if(!empty($db->get_error())){
	...
	$data->insert1($input->FilterText($_SESSION['settings']['admin_username']),$password,$input->FilterText($_SESSION['settings']['admin_email']));
	if(!empty($db->get_error())){
		echo '<div class="installer-error"><div class="rounded rounded-red">'.htmlspecialchars($db->get_error(), ENT_COMPAT, 'UTF-8').'</div></div>';
		return false;
	}
	...
	if(empty($id)){
		echo '<div class="installer-error"><div class="rounded rounded-red">Could not create administrator account.</div></div>';
		return false;
	}
