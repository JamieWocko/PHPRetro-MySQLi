...
class HoloInput {
	...
	function FilterText($str) {
		...
		if(isset($GLOBALS['db']) && is_object($GLOBALS['db']) && method_exists($GLOBALS['db'], 'escape')){
			$str = $GLOBALS['db']->escape($str);
		}else{
			$str = addslashes($str);
		}
		return $str;
...
class HoloUser {
	...
	function __construct($name = null, $password = null, $updateuser = false, $rememberme = null){
		$this->HoloUser($name, $password, $updateuser, $rememberme);
	}
	...
	function avatarURL($figure,$style,$return = 0){
		...
		if($GLOBALS['settings']->find("site_cache_images") == "1" && file_exists("./cache/avatars/".$figure.",".$expandedstyle.",".$hash.".png")){
			...
			$URL = "http://www.habbo.com/habbo-imaging/avatarimage?figure=".$figure."&size=".$style[0]."&direction=".$style[1]."&head_direction=".$style[2]."&crr=".$style[5]."&gesture=".$style[3]."&frame=".$style[4];
	...
	function HCDaysLeft($id){
		...
		if($GLOBALS['serverdb']->num_rows($GLOBALS['core']->select8($id)) > 0){
			...
			$day = isset($tmp[0]) ? (int) $tmp[0] : 0;
			$month = isset($tmp[1]) ? (int) $tmp[1] : 0;
			$year = isset($tmp[2]) ? (int) $tmp[2] : 0;
	...
	function GiveHC($id, $months){
		...
		if($GLOBALS['serverdb']->result($GLOBALS['core']->select9($id)) > 0){
			...
			$d = date('d');
			$Y = date('Y');
			$date = date('d-m-Y', mktime(0, 0, 0, (int) $m, (int) $d, (int) $Y));
	...
	function HoloDatabase($conn){
		switch($conn['server']){
			case "mysql":
				...
				$port = !empty($conn['port']) ? (int) $conn['port'] : ini_get("mysqli.default_port");
				$this->connection = mysqli_init();
				if($this->connection instanceof mysqli){
					try{
						$this->connection->real_connect($conn['host'], $conn['username'], $conn['password'], $conn['database'], $port);
					}catch(Throwable $e){
...
class mysql extends HoloDatabase {
	function query($query){
		...
		if(!($this->connection instanceof mysqli)){ return false; }
		try{
	...
	function fetch_assoc($query){
		...
		if(defined('DEBUG')){ $error = $this->get_error(); if($result == false && !empty($error)){ echo $error . "<br />Query that errored: ".$this->lastquery; } }
		return $result;
	}
	...
	function fetch_array($result,$result_type=0){
		...
		if($result_type === 0){ $result_type = MYSQLI_BOTH; }
		$result = mysqli_fetch_array($result,$result_type);
		if(defined('DEBUG')){ $error = $this->get_error(); if($result == false && !empty($error)){ echo $error . "<br />Query that errored: ".$this->lastquery; } }
		return $result;
	}
	...
	function result($query,$row=0,$column=0){
		$result = false;
		if($query !== false && mysqli_data_seek($query, $row)){
			$data = mysqli_fetch_array($query, MYSQLI_BOTH);
			$result = isset($data[$column]) ? $data[$column] : false;
		}
		if(defined('DEBUG')){ if($result == false){ echo $this->get_error() . "<br />Query that errored: ".$this->lastquery; } }
		return $result;
	}
	function insert_id($query=null){
		if(!($this->connection instanceof mysqli)){ return 0; }
...
class HoloFigureCheck {
	...
	function __construct($figure=null,$gender=null,$club=false){
		if(func_num_args() > 0){
			$this->HoloFigureCheck($figure, $gender, $club);
		}
	}
...
class HoloSettings {
	...
	function find($key){
		$value = null;
		if(!empty($this->cache) && isset($this->cache[$key])){
			$value = $this->cache[$key];
		}else{
			$sql = $GLOBALS['db']->query("SELECT value FROM ".PREFIX."settings WHERE id = '".$key."' LIMIT 1");
			if($sql !== false){
				$dbValue = $GLOBALS['db']->result($sql);
				if($dbValue !== false && $dbValue !== null && $dbValue !== ""){
					$value = $dbValue;
				}
			}
		}
		if(($value === null || $value === false || $value === "") && isset($this->defaults[$key])){
			$value = $this->defaults[$key];
		}
		return $GLOBALS['input']->HoloText((string) $value, true);
	}
	function checkCache(){
		if($this->find("cache_settings") == "1"){
