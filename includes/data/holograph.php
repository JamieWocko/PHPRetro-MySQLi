...
function holograph_column_exists($table, $column){
	static $cache = array();
	$key = $table.'.'.$column;
	if(isset($cache[$key])){ return $cache[$key]; }
	$sql = @$GLOBALS['serverdb']->query("SHOW COLUMNS FROM `".$table."` LIKE '".$column."'");
	$cache[$key] = ($sql !== false && $GLOBALS['serverdb']->num_rows($sql) > 0);
	return $cache[$key];
}

function holograph_remote_addr(){
...
class installer_sql {
	function alter1(){
		...
		$sql = @$GLOBALS['serverdb']->query("SHOW COLUMNS FROM `users` LIKE 'hc_before'");
		if($sql === false || $GLOBALS['serverdb']->num_rows($sql) < 1){
	...
	function insert1($username,$password,$email = ''){
		$columns = array('name','password','`rank`','email','birth','hbirth','figure','sex','mission','credits');
		$values = array(
			"'".$username."'",
			"'".$password."'",
			"'7'",
		...
		if(holograph_column_exists('users', 'guideavailable')){
			$columns[] = 'guideavailable';
			$values[] = "'0'";
		}
		if(holograph_column_exists('users', 'badge_status')){
			$columns[] = 'badge_status';
			$values[] = "'1'";
		}
		$GLOBALS['serverdb']->query("INSERT INTO users (".implode(',', $columns).") VALUES (".implode(',', $values).")");
	}
	function insert2($id,$username,$email){
		$GLOBALS['serverdb']->query("INSERT INTO ".PREFIX."users (id,name,ipaddress_last,lastvisit,online,newsletter,email_verified,show_home,email_friendrequest,email_minimail,show_online,email) VALUES ('".$id."','".$username."','".holograph_remote_addr()."','".time()."','".time()."','1','1','1','1','1','1','".$email."')");
	}
	function select1(){
		// Test query to verify that a valid Holograph database exists.
		$sql = $GLOBALS['serverdb']->query("SELECT 'passed' AS `test` FROM `system` LIMIT 1");
		return $sql;
	}
}
class core_sql {
	...
	function select2($id){
		$sql = $GLOBALS['serverdb']->query("SELECT descr,date_expire FROM users_bans WHERE userid = '".$id."' OR ipaddress = '".holograph_remote_addr()."' LIMIT 1");
		return $sql;
	}
	function delete1($id){
		$GLOBALS['serverdb']->query("DELETE FROM users_bans WHERE userid = '".$id."' OR ipaddress = '".holograph_remote_addr()."' LIMIT 1");
	}
	function select3($id) {
	...
	function update6($id,$time){
		$GLOBALS['serverdb']->query("UPDATE users SET lastvisit = '".$time."', ipaddress_last = '".holograph_remote_addr()."' WHERE id = '".$id."' LIMIT 1");
	}
}
class index_sql {
...
class register_sql {
	...
	function insert1($name,$password,$dob,$figure,$gender,$scredits){
		$date = HoloDate();
		$columns = array('name','password','birth','figure','sex','`rank`','hbirth','tickets','credits');
		$values = array(
			"'".$name."'",
			"'".$password."'",
			"'".$dob."'",
...
class housekeeping_sql {
	...
	function select2(){
		$sql = $GLOBALS['serverdb']->query("SELECT onlinecount FROM `system` UNION ALL SELECT activerooms FROM `system`");
		return $sql;
	}
