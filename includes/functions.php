...
function HoloDate(){
	...
	$date['month'] = $date['m'];
	$date['year'] = $date['Y'];
	$date['date_normal'] = date('d-m-Y', mktime(0, 0, 0, (int) $date['m'], (int) $date['d'], (int) $date['Y']));
	$date['date_reversed'] = date('Y-m-d', mktime(0, 0, 0, (int) $date['m'], (int) $date['d'], (int) $date['Y']));
...
if(!function_exists('socket_create') || !function_exists('socket_connect') || !function_exists('socket_send')){
	return false;
}
...
$sock = socket_create(AF_INET, SOCK_STREAM, getprotobyname('tcp'));
	...
	if(@socket_send($sock, $data, strlen($data), MSG_DONTROUTE) === false){
		@socket_close($sock);
		return false;
	}
	@socket_close($sock);
