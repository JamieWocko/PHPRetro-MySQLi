...
$username = isset($_GET['username']) ? $input->HoloText($_GET['username']) : "";
$rememberme = isset($_GET['rememberme']) ? $input->HoloText($_GET['rememberme']) : "";
$pageto = isset($_GET['page']) ? $input->HoloText($_GET['page']) : "";
...
<?php } ?>
                ...
                <li class="no-label">
                    <input tabindex="3" type="checkbox" value="true" name="_login_remember_me" id="login-remember-me"<?php if(isset($_GET['rememberme']) && $rememberme == "true"){ echo " checked=\"checked\""; } ?>/>
...
echo "	    <ul class=\"tag-list\">";
	$row = array();
	$tags = array();
	while($tagRow = $db->fetch_array($sql, 1)){
		if(!is_array($tagRow) || !isset($tagRow['tag']) || $tagRow['tag'] === ""){
			continue;
		}
	...
	foreach($row as $tagRow){
		$tag = $tagRow['tag'];
		$count = isset($tagRow['quantity']) ? $tagRow['quantity'] : 0;
		$tags[$tag] = $count;
	}
		
		if(empty($tags)){ echo $lang->loc['no.tags']; }else{
