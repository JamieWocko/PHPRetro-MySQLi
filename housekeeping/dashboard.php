...
<?php echo $lang->loc['users.online']; ?>: <?php echo isset($stats[6]) ? $stats[6] : 0; ?><br />
<?php echo $lang->loc['active.rooms']; ?>: <?php echo isset($stats[7]) ? $stats[7] : 0; ?><br />
</div>
</td>
...
<div class="center">
	...
	<?php if($settings->checkCache() == true && $user->user("rank") > 5){ ?><div class="clean-yellow"><?php echo $lang->loc['cache.outdated']; ?> <a href="<?php echo PATH; ?>/housekeeping/cache" /><?php echo $lang->loc['update.now']; ?> &raquo;</a></div><?php } ?>
<?php
$currentMonth = date('m');
$currentYear = date('Y');
$currentMonthTime = mktime(0,0,0,$currentMonth,1,$currentYear);
$nextMonthTime = strtotime("+1 Month",$currentMonthTime);
if($nextMonthTime - 60*60*24*7 < time()){
	$sql = $db->query("SELECT COUNT(*) FROM ".PREFIX."collectables WHERE date > '".time()."' AND date <= '".$nextMonthTime."' LIMIT 1");
	if($db->result($sql) == 0){
