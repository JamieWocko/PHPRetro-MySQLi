...
if($do == "save"){
	...
	if(empty($error)){
		...
		if(isset($row['help']) && $row['help'] == "true"){ header('Location: '.PATH.'/housekeeping/help'); }
...
if(isset($_GET['help']) && $_GET['help'] == "true"){ $content .= '<input type="hidden" name="help" value="true" />'; }
$content .= 
...
<input type="checkbox" style="width: auto" name="site" value="1"'.$checked[0].' title="'.$lang->loc['where.desc'].'" /><label for="site">'.$lang->loc['site'].'</label>&nbsp;&nbsp;<input type="checkbox" style="width: auto" name="hotel" value="1"'.$checked[1].' title="'.$lang->loc['where.desc'].'" /><label for="hotel">'.$lang->loc['hotel'].'</label><br />
...
<form name="settings" action="'.PATH.'/housekeeping/alerts?type='.$input->FilterText($type).'&do=remove" method="POST">
