...
$requestedPage = isset($_GET['page']) ? $input->FilterText($_GET['page']) : "";
$sql = $db->query("SELECT * FROM ".PREFIX."settings_pages WHERE id = '".$requestedPage."' LIMIT 1");
		...
		switch($row2['type']){
			...
			case "radiobuttons":
				...
				foreach($values as $value){
					$value = explode(",",$value);
					$checked = "";
					if($value[0] == $row2['value']){ $checked = ' checked="true"'; }
					echo '<input type="radio" class="radio" name="'.$row2['id'].'" id="'.$row2['id'].'" value="'.$value[0].'"'.$checked.' title="'.$input->HoloText($row2['description'],true).'">'.$value[1].'</input> ';
				}
				echo "<br />\n";
				break;
			case "selectbox":
				...
				foreach($values as $value){
					$value = explode(",",$value);
					$selected = "";
					if($value[0] == $row2['value']){ $selected = ' selected="true"'; }
