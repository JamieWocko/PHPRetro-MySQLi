...
$currentMonth = date('m');
$currentYear = date('Y');
$currentTime = mktime(0,0,0,$currentMonth,1,$currentYear);

$furni_id = $serverdb->result($data->select3($currentTime), 0, 3);
...
$message = $lang->loc['collectable.bought']." ".$input->HoloText($serverdb->result($data->select3($currentTime), 0)).".";
}else{
