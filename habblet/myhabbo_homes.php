...
if($db->num_rows($sql) == 0 && $targetId == $user->id){ $db->query("INSERT INTO ".PREFIX."homes_edit (pageid,editorid,`type`,time) VALUES ('".$user->id."','".$user->id."','user','".time()."')"); }
break;
case "cancel":
...
break;
case "save":
$note = explode("/", isset($_POST['stickienotes']) ? $_POST['stickienotes'] : "");
$widget = explode("/", isset($_POST['widgets']) ? $_POST['widgets'] : "");
$sticker = explode("/", isset($_POST['stickers']) ? $_POST['stickers'] : "");
$background = explode(":", isset($_POST['background']) ? $_POST['background'] : "");
