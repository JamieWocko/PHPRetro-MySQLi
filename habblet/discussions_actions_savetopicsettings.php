...
$name = isset($_POST['topicName']) ? $input->FilterText($_POST['topicName']) : "";
$closed = isset($_POST['topicClosed']) ? $input->FilterText($_POST['topicClosed']) : "0";
$sticky = isset($_POST['topicSticky']) ? $input->FilterText($_POST['topicSticky']) : "0";
$pagenum = isset($_POST['page']) ? (int) $input->FilterText($_POST['page']) : 1;
$pagenum = ($pagenum < 1) ? 1 : $pagenum;
if($closed == "1"){ $open = "0"; }else{ $open = "1"; }
...
if($memberrow[2] > 1){ $db->query("UPDATE ".PREFIX."forum_threads SET open = '".$open."', sticky = '".$sticky."' WHERE id = '".$threadrow['id']."' LIMIT 1"); }
if($memberrow[2] > 1 || $threadrow['starterid'] == $user->id){ $db->query("UPDATE ".PREFIX."forum_threads SET title = '".$name."' WHERE id = '".$threadrow['id']."' LIMIT 1"); }
