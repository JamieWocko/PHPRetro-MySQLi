...
$id = isset($_POST['groupId']) ? $input->FilterText($_POST['groupId']) : "";
$topicid = isset($_POST['topicId']) ? $input->FilterText($_POST['topicId']) : "";
$pagenum = isset($_POST['page']) ? (int) $input->FilterText($_POST['page']) : 1;
$pagenum = ($pagenum < 1) ? 1 : $pagenum;
$postid = isset($_POST['postId']) ? $input->FilterText($_POST['postId']) : "";
...
$postrow = $db->fetch_assoc($db->query("SELECT * FROM ".PREFIX."forum_posts WHERE id = '".$postid."' LIMIT 1"));

if($postrow['posterid'] != $user->id || $memberrow[2] < 2){ exit; }

$total = $db->result($db->query("SELECT COUNT(*) FROM ".PREFIX."forum_posts WHERE threadid = '".$threadrow['id']."'"));

if($total > 1){
	$firstpost = $db->result($db->query("SELECT id from ".PREFIX."forum_posts ORDER BY id ASC LIMIT 1"));
	if($firstpost == $postrow['id']){
		$db->query("UPDATE ".PREFIX."forum_posts SET message = '".$lang->loc['post.deleted']."', edit_time = '".time()."' WHERE id = '".$postrow['id']."' LIMIT 1");
	}else{
		$db->query("DELETE FROM ".PREFIX."forum_posts WHERE id = '".$postrow['id']."' LIMIT 1");
	}
}else{
	$db->query("DELETE FROM ".PREFIX."forum_threads WHERE id = '".$threadrow['id']."' LIMIT 1");
	$db->query("DELETE FROM ".PREFIX."forum_posts WHERE id = '".$postrow['id']."' LIMIT 1");
	echo "TOPIC_DELETED"; exit;
}
...
<?php if($user->id != 0 && ((($grouprow[9] == 2 && $memberrow[2] > 1) || ($grouprow[9] == 1 && $memberrow[2] > 0)) || ($grouprow[9] == 0)) && $threadrow['open'] == "1"){ ?>
                    ...
                    <input type="hidden" id="email-verfication-ok" value="<?php echo $user->user("email_verified"); ?>"/>
