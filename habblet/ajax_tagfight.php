...
$sql = $db->query("SELECT COUNT(id) FROM ".PREFIX."tags WHERE tag = '".$input->FilterText($tag1)."'");
$tag_1_results = $db->result($sql);
$sql = $db->query("SELECT COUNT(id) FROM ".PREFIX."tags WHERE tag = '".$input->FilterText($tag2)."'");
$tag_2_results = $db->result($sql);

$tag_1 = $input->HoloText($tag1);
$tag_2 = $input->HoloText($tag2);

if($tag_1_results == $tag_2_results){

$end = 0;
} elseif($tag_1_results > $tag_2_results){
$tag_1 = "<b>" . $input->HoloText($tag1) . "</b>";
$end = 2;
} elseif($tag_1_results < $tag_2_results){
$tag_2 = "<b>" . $input->HoloText($tag2) . "</b>";
$end = 1;
}
