...
$page['bypass'] = isset($page['bypass']) ? $page['bypass'] : false;
...
$requestLabel = isset($_POST['label']) ? $_POST['label'] : "inbox";
$requestConversationId = isset($_POST['conversationId']) ? $_POST['conversationId'] : "";

switch($requestLabel){ case "sent": $sql = $db->query("SELECT COUNT(*) FROM ".PREFIX."minimail WHERE senderid = '".$user->id."'"); break;
case "trash": $sql = $db->query("SELECT COUNT(*) FROM ".PREFIX."minimail WHERE to_id = '".$user->id."' AND deleted = '1'"); break;
case "conversation": $sql = $db->query("SELECT COUNT(*) FROM ".PREFIX."minimail WHERE (to_id = '".$user->id."' OR senderid = '".$user->id."') AND conversationid = '".$input->FilterText($requestConversationId)."'"); break;
...
if(isset($_POST['label']) || $page['bypass'] == true){
	if(!isset($label)){ $label = isset($_POST['label']) ? $input->FilterText($_POST['label']) : "inbox"; }
	if(!isset($start)){ $start = isset($_POST['start']) ? $_POST['start'] : null; }
	$conversationid = isset($_POST['conversationId']) ? $_POST['conversationId'] : null;
	$unread = isset($_POST['unreadOnly']) ? $_POST['unreadOnly'] : null;
	?>
...
case "conversation": ?>
	...
	<?php $id = isset($_POST['messageId']) ? $input->FilterText($_POST['messageId']) : "";
	$conid = isset($_POST['conversationId']) ? $input->FilterText($_POST['conversationId']) : ""; ?>
