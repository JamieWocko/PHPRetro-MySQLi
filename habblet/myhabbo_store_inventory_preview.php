...
$id = isset($_POST['itemId']) ? $input->FilterText($_POST['itemId']) : "";
$type = isset($_POST['type']) ? $input->FilterText($_POST['type']) : "stickers";
...
if($type == 2){
$sql = $db->query("SELECT id,data,name,`desc` FROM ".PREFIX."homes_catalogue WHERE id = '".$id."'");
}else{
...
header('X-JSON: ["'.formatItem($type,$row[1],true).'",'.$json['preview'].',"'.$description.'",'.$json['type'].','.$json['widget'].',1]');
}else{
