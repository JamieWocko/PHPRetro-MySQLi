...
$id = isset($_GET['id']) ? $input->FilterText($_GET['id']) : 0;
...
if(!isset($_SESSION['threadviewed']) || !is_array($_SESSION['threadviewed'])){ $_SESSION['threadviewed'] = array(); }
if(!isset($_SESSION['threadviewed'][$threadrow['id']]) || $_SESSION['threadviewed'][$threadrow['id']] != true){ $db->query("UPDATE ".PREFIX."forum_threads SET views = views + 1 WHERE id = '".$threadrow['id']."' LIMIT 1"); $_SESSION['threadviewed'][$threadrow['id']] = true; }
?>
...
<?php
$sql = $db->query("SELECT * FROM ".PREFIX."forum_threads WHERE groupid = '".$id."' ORDER BY sticky DESC, time DESC LIMIT 10 OFFSET ".$offset);
$i = 0;
while($row = $db->fetch_assoc($sql)){
if($input->IsEven($i)){ $even = "even"; }else{ $even = "odd"; }
